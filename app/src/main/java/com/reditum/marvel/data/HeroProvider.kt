...
object HeroProvider {
    private val heroesFlow: MutableStateFlow<List<Character>> = MutableStateFlow(emptyList())

    fun getHeroes(): MutableStateFlow<List<Character>> {
        if (heroesFlow.value.isEmpty()) {
            heroesFlow.update {
                listOf(
                    Character(
                        1,
